...
type View = "overview" | "inventory" | "discoveries" | "cbom" | "posture" | "cyber-rating" | "reporting";

const navItems: { label: string; icon: React.FC<{ className?: string }>; view: View }[] = [
  { label: "Overview",       icon: Radar,               view: "overview"     },
  { label: "Asset Inventory",icon: Layers,              view: "inventory"    },
  { label: "Discoveries",    icon: Search,              view: "discoveries"  },
  { label: "CBOM",           icon: Cpu,                 view: "cbom"         },
  { label: "Crypto Posture", icon: Shield,              view: "posture"      },
  { label: "Cyber Rating",   icon: Star,                view: "cyber-rating" },
  { label: "Reporting",      icon: ChartNoAxesCombined, view: "reporting"    },
];

const viewTitles: Record<View, string> = {
  "overview":     "QuantWarden Security Overview",
  "inventory":    "Asset Inventory",
  "discoveries":  "Asset Discoveries",
  "cbom":         "Cryptographic Bill of Materials",
  "posture":      "Crypto Posture of PQC",
...
export default function Home() {
  ...
  /* ── theme tokens ── */
  const t = darkMode ? {
    pageBg:        "bg-[radial-gradient(circle_at_15%_0%,_rgba(14,165,233,0.15),_transparent_40%),radial-gradient(circle_at_85%_10%,_rgba(99,102,241,0.16),_transparent_40%),linear-gradient(180deg,#050b14_0%,#07111f_100%)]",
    ...
    navActive:     "bg-cyan-500/15 text-cyan-100 shadow-[inset_0_0_0_1px_rgba(34,211,238,0.35)]",
  ...
  return (
    <div className={`${darkMode ? "theme-dark" : "theme-light"} min-h-screen ${t.pageBg} text-slate-100`}>
      <div className="mx-auto grid min-h-screen w-full max-w-[1600px] grid-cols-1 lg:grid-cols-[272px_1fr]">

        {/* ── Sidebar ── */}
        <aside className={`border-b ${t.sidebarBorder} ${t.sidebarBg} lg:border-r lg:border-b-0 flex flex-col`}>
          {/* Logo */}
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.label}
                  {item.view === "cyber-rating" && (
                    <span className={`ml-auto rounded-full border px-1.5 py-0.5 text-[10px] font-medium ${t.badge}`}>755</span>
                  )}
                </button>
              );
            })}
          ...
          <div className="px-4 mb-3">
            ...
            >
              {darkMode
                ? <><Sun className={`h-4 w-4 ${t.toggleText}`} /><span className={t.toggleText}>Light Mode</span></>
                : <><Moon className={`h-4 w-4 ${t.toggleText}`} /><span className={t.toggleText}>Dark Mode</span></>
              }
            </button>
          </div>
        ...
        {/* ── Main ── */}
        <main className="flex flex-col p-4 md:p-7 overflow-y-auto">
          {/* Page header */}
          <header className={`mb-6 flex flex-col gap-4 rounded-2xl border ${t.headerCard} p-5 md:flex-row md:items-center md:justify-between shrink-0`}>
            <div>
              <p className={`text-xs uppercase tracking-[0.2em] ${t.headerSub}`}>QuantWarden</p>
              <h1 className={`mt-1 text-2xl font-semibold ${t.headerTitle}`}>{viewTitles[activeView]}</h1>
            </div>
          ...
          <div className="flex-1">
            ...
            {activeView === "inventory"    && <AssetInventory />}
            {activeView === "discoveries"  && <Discoveries />}
            {activeView === "cbom"         && <Cbom />}
            {activeView === "posture"      && <Posture />}
            {activeView === "cyber-rating" && <CyberRating />}
